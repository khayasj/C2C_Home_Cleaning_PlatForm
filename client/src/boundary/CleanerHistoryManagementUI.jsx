...
function CleanerHistoryManagementUI() {
  ...
  const viewHistory = async () => {
    try {
      const res = await fetch(`http://localhost:5000/api/cleaner/history/all/${usernameStored}`);
  ...
  const searchHistory = async () => {
    try {
      ...
      const res = await fetch(`http://localhost:5000/api/cleaner/history/${usernameStored}${query}`);
      const data = await res.json();
      setServiceList(data);
    } catch (err) {
      console.error('Failed to fetch services:', err);
        ...
        <div className="search-section">
          <CustomTable
            columns={[
              ...
              { id: 'homeownerusername', label: 'Homeowner username' },
...
export default CleanerHistoryManagementUI;
